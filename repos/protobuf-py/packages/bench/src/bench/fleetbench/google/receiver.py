...
"""Message operations for the upstream ``google.protobuf`` API.
...
class Ops:
    """Idiomatic message operations (C++ ``receiver.h``) for google.protobuf."""
...
# - ByteSize:    a cheap non-allocating count in C++/google with no Buf
#                protobuf equivalent (users don't need it), so omitted to keep
